...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='ml-1m', #pinterest-20
...
def training(flag, model, dataset, epochs, num_negatives):
    ...
    with tf.Session(config=config) as sess:
        ...
        batch_begin = time()
        batches = bg.shuffle(dataset, model.batch_choice, num_negatives)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename=os.path.join(log_dir, "log_%s_dataset_%s_lr%.2f_reg%.0e_%s" %
                                              (algo, args.dataset, args.lr, regs[2],
                                               strftime('%Y_%m_%d_%H-%M-%S', localtime()))), level=logging.INFO)
